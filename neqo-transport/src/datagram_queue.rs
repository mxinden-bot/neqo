...
pub enum DatagramQueueOutcome {
    ...
    /// larger than what any one eviction frees. `evicted` is the total
    /// number evicted, tracked or not, for aggregate stats; `dropped` holds
    /// only the tracking ID of each evicted datagram that had one (an
    /// untracked eviction needs no further caller action, so it contributes
    /// nothing to `dropped` - `dropped` can be shorter than `evicted`, or
    /// empty, even though eviction happened), for the caller to report a
    /// [`DatagramOutcome::Dropped`] event for each one separately.
    Overflowed {
        dropped: Vec<DatagramId>,
        evicted: usize,
    },
}
...
pub struct DatagramQueueCapacity {
    ...
    /// reached. Charged bytes, not payload bytes: each datagram costs its
    /// payload length plus a fixed per-entry overhead, so this is an upper
    /// bound on the payload a grant may cover, not an exact allowance.
...
impl GroupQueue {
    ...
    fn push(&mut self, send_order: SendOrder, dgram: QueuedDatagram) {
        // Sized for one entry: with distinct `send_order` values per datagram
        // the default capacity-4 growth would cost ~4x `PER_DATAGRAM_OVERHEAD`
        // per bucket, which the byte budget does not account for.
        self.by_order
            .entry(send_order)
    ...
    ///
    /// Appends one entry per expired datagram to `ids` (`Some(id)` for
    /// tracked ones) and returns the total bytes freed, rather than
    /// returning the expired datagrams themselves: the caller only ever
    /// needs their ids and charged size, so there is no reason to keep their
    /// payload data alive past this call.
    fn expire_old(
        &mut self,
        now: Instant,
        max_age: Duration,
        ids: &mut Vec<Option<DatagramId>>,
    ) -> usize {
        let mut freed = 0;
...
/// kind.
///
/// `send_order` here is a bare [`SendOrder`], with no way to express a null send order.
/// [`crate::send_stream::SendStreams`]' analogous `OrderGroup` instead models it as
/// `Option<SendOrder>` and serves the null bucket ahead of every `Some` bucket, per the spec's
/// must-not-starve rule for null-`[[SendOrder]]` sends - not yet mirrored here, a known gap.
#[derive(Debug)]
...
impl DatagramQueue {
    ...
    /// Remove and return every datagram that has reached `max_age`; the
    /// boundary is inclusive. Returns one entry per expired datagram,
    /// `Some(id)` for tracked ones, in ascending `send_order` per send-group
    /// and FIFO within each order - not in global age order.
    #[must_use]
    pub fn expire(&mut self, now: Instant, default_max_age: Duration) -> Vec<Option<DatagramId>> {
        ...
        self.groups.retain(|_, group| {
            freed_bytes += group.expire_old(now, max_age, &mut ids);
            !group.is_empty()
        });
    ...
    /// `group_id` (lowest first) for determinism. `SendGroupId::new(0)`, the
    /// null-sendGroup sentinel, therefore sorts first at equal `send_order`,
    /// so null-group datagrams are systematically the first eviction victims
    /// in a tie.
    ///
    ...
    pub fn enqueue(
        ...
        // Shed spare capacity so `charge(data.len())` covers the whole
        // allocation rather than just the payload.
        data.shrink_to_fit();
        ...
        'evict: while self.total_bytes + new_charge > self.max_queued_bytes {
            ...
            while let Some(dgram) = self.evict_at(victim_group, victim_order) {
                evicted_any = true;
                evicted_count += 1;
                dropped.extend(dgram.id);
                if self.total_bytes + new_charge <= self.max_queued_bytes {
                    break 'evict;
                }
            }
        }
        if outranked && !evicted_any {
            qdebug!(
            ...
            // outcome but `Ok`: arm one, or it waits forever. Gated on
            // `new_charge`, the exact charge this write needs, so a retry of
            // this same write isn't told to resume before there is actually
            // room for it.
            self.blocked = Some(new_charge);
        ...
        let outcome = if evicted_any {
            self.blocked = Some(1);
            DatagramQueueOutcome::Overflowed {
                dropped,
                evicted: evicted_count,
            }
        } else if self.below_watermark() {
            DatagramQueueOutcome::Ok
        } else {
            // The datagram was admitted, so any freed byte is progress
            // for the next write, not a specific charge to wait for.
            self.blocked = Some(1);
    ...
    /// from a specific group ([`Self::evict_lowest_priority`],
    /// [`Self::take_next`]).
    fn finish_removal(
        &mut self,
    ...
    /// water mark or byte budget: `high_water_mark == Some(0)` or
    /// `max_queued_bytes == 0` would otherwise wedge the sender permanently,
    /// since neither bound is ever satisfied by a nonnegative count/byte
    /// total, not even at zero.
    ///
...
mod tests {
    ...
    #[test]
    fn datagram_outcome_id_returns_the_tracking_id() {
        assert_eq!(DatagramOutcome::Expired(7).id(), 7);
        assert_eq!(DatagramOutcome::Dropped(9).id(), 9);
    }
    ...
    #[test]
    fn enqueue_shrinks_spare_capacity_so_the_charge_reflects_real_memory() {
        // charge() bills data.len(), so a Vec holding unused capacity (e.g.
        // a caller that over-allocates then fills only part of it) would
        // otherwise let the queue hold more real memory than max_queued_bytes
        // accounts for.
        let mut q = DatagramQueue::default();
        let t = now();
        ...
        drop(q.enqueue(data, Some(1), t, g(0), 0));

        let taken = q.take_next().expect("a datagram is queued");
        assert_eq!(
            taken.data.capacity(),
            taken.data.len(),
            "spare capacity must be shed so charge(len()) reflects the real allocation"
        );
    }
    ...
    fn capacity_tracks_bytes_and_count() {
        ...
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 0));
        assert_eq!(
            q.capacity(),
            DatagramQueueCapacity {
    ...
    fn drain_basic() {
        ...
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 0));
        drop(q.enqueue(vec![0, 2], Some(2), t, g(0), 0));
    ...
    fn hard_limit_untracked_datagram_drops_silently() {
        ...
        drop(queue.enqueue(vec![1], None, t, g(0), 0));
        ...
        assert_eq!(
            ...
            "an untracked eviction needs no caller action, so it is not in dropped, \
             but it still counts toward evicted"
        );
    ...
    #[test]
    fn zero_high_water_mark_resumes_once_the_queue_drains_empty() {
        // `total_count < mark` is never true when the mark is zero, even
        // once the queue is empty: `resume_if_unblocked` also has to treat
        // an empty queue as under the watermark, or a blocked sender waits
        // forever.
        let mut queue = DatagramQueue::default();
        queue.set_high_water_mark(Some(0));
        let t = now();
        ...
        assert!(
            queue.resume_if_unblocked(),
            "an empty queue must be treated as under the watermark even when the watermark itself is zero"
        );
    }
    ...
    fn max_age_expiration() {
        let mut queue = DatagramQueue::default();
        let t0 = now();
        drop(queue.set_max_age(Some(Duration::from_millis(100)), t0, NO_DEFAULT));
        drop(queue.enqueue(vec![1], Some(1), t0, g(0), 0));
    ...
    fn max_age_expiration_untracked_datagram_reports_nothing() {
        ...
        drop(queue.enqueue(vec![1], None, t0, g(0), 0));
    ...
    fn next_expiry_is_none_when_the_deadline_overflows() {
        ...
        let mut queue = DatagramQueue::default();
        let t = now();
        drop(queue.enqueue(vec![1], Some(1), t, g(0), 0));
        assert_eq!(queue.next_expiry(NO_DEFAULT), None);
    }
    ...
    fn next_expiry_tracks_the_oldest_datagram_across_groups() {
        let mut queue = DatagramQueue::default();
        let t0 = now();
        drop(queue.set_max_age(Some(Duration::from_millis(50)), t0, NO_DEFAULT));
        ...
        drop(queue.enqueue(vec![2], Some(2), t1, g(1), 0));
    ...
    fn next_expiry_advances_once_the_oldest_datagram_is_gone() {
        ...
        drop(queue.enqueue(vec![1], Some(1), t0, g(0), 0));
        let t1 = t0 + Duration::from_millis(10);
        drop(queue.enqueue(vec![2], Some(2), t1, g(0), 0));
    ...
    fn drain() {
        ...
        drop(queue.enqueue(vec![0, 1], Some(1), t, g(0), 0));
        drop(queue.enqueue(vec![0, 2], None, t, g(0), 0));
    ...
    fn drain_reports_every_expired_datagram() {
        ...
        let mut q = DatagramQueue::default();
        let t0 = now();
        drop(q.set_max_age(Some(Duration::from_millis(50)), t0, NO_DEFAULT));

        drop(q.enqueue(vec![1], Some(1), t0, g(0), 0));
        drop(q.enqueue(vec![2], None, t0, g(0), 0));
    ...
    fn priority_order_within_group() {
        ...
        let mut q = DatagramQueue::default();
        let t = now();
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 10)); // order 10
        drop(q.enqueue(vec![0, 2], Some(2), t, g(0), 30)); // order 30 (highest)
        drop(q.enqueue(vec![0, 3], Some(3), t, g(0), 20)); // order 20
    ...
    fn fifo_within_same_order() {
        ...
        let t = now();
        drop(q.enqueue(vec![0, 10], Some(10), t, g(0), 5));
        drop(q.enqueue(vec![0, 11], Some(11), t, g(0), 5));
        drop(q.enqueue(vec![0, 12], Some(12), t, g(0), 5));
    ...
    fn priority_mixed_orders_same_group() {
        let mut q = DatagramQueue::default();
        let t = now();
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 1));
        drop(q.enqueue(vec![0, 2], Some(2), t, g(0), 3));
        drop(q.enqueue(vec![0, 3], Some(3), t, g(0), 1)); // same as id=1
        drop(q.enqueue(vec![0, 4], Some(4), t, g(0), 3)); // same as id=2
    ...
    fn round_robin_two_groups() {
        ...
        let t = now();
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 0)); // group A
        drop(q.enqueue(vec![0, 2], Some(2), t, g(1), 0)); // group B
        drop(q.enqueue(vec![0, 3], Some(3), t, g(0), 0)); // group A
        drop(q.enqueue(vec![0, 4], Some(4), t, g(1), 0)); // group B
        drop(q.enqueue(vec![0, 5], Some(5), t, g(0), 0)); // group A
    ...
    fn round_robin_priority_across_groups() {
        ...
        let mut q = DatagramQueue::default();
        let t = now();
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 10));
        drop(q.enqueue(vec![0, 2], Some(2), t, g(0), 5));
        drop(q.enqueue(vec![0, 3], Some(3), t, g(1), 20));
        drop(q.enqueue(vec![0, 4], Some(4), t, g(1), 1));
    ...
    fn round_robin_three_groups() {
        ...
        drop(q.enqueue(vec![0, 1], Some(1), t, g(10), 0));
        drop(q.enqueue(vec![0, 2], Some(2), t, g(20), 0));
        drop(q.enqueue(vec![0, 3], Some(3), t, g(30), 0));
    ...
    fn clearing_max_age_restores_the_default() {
        ...
        drop(q.set_max_age(Some(Duration::from_millis(100)), t0, default));
        // outgoingMaxAge = null; 0 is a RangeError at the setter and never gets here.
        drop(q.set_max_age(None, t0, default));
        drop(q.enqueue(vec![1], Some(1), t0, g(0), 0));
    ...
    fn lowering_max_age_sheds_already_queued_datagrams() {
        // set_max_age's own expire() tail is the whole reason it returns a
        // Vec - every other call site in this suite sets max_age before
        // anything is queued, so this pins that tightening `outgoingMaxAge`
        // sheds what is already queued, at the setter, not just on the next
        // `expire`.
        let mut q = DatagramQueue::default();
        let t0 = now();
        drop(q.set_max_age(Some(Duration::from_millis(100)), t0, NO_DEFAULT));
        drop(q.enqueue(vec![1], Some(1), t0, g(0), 0));

        let expired = q.set_max_age(
    ...
    fn explicit_max_age_is_clamped_to_the_1ms_floor() {
        ...
        let t0 = now();
        let default = Duration::from_millis(20);
        drop(q.set_max_age(Some(Duration::from_micros(1)), t0, default));
        drop(q.enqueue(vec![1], Some(1), t0, g(0), 0));
    ...
    fn explicit_max_age_above_default_is_not_clamped() {
        ...
        let default = Duration::from_millis(20);
        drop(q.set_max_age(Some(Duration::from_millis(300)), t0, default));
        drop(q.enqueue(vec![1], Some(1), t0, g(0), 0));
    ...
    fn drain_stops_at_budget() {
        ...
        let t = now();
        for i in 1..=5 {
            drop(q.enqueue(vec![0, i], Some(u64::from(i)), t, g(0), 0));
        }
    ...
    fn drain_budget_zero_keeps_everything() {
        let mut q = DatagramQueue::default();
        let t = now();
        drop(q.enqueue(vec![1], Some(1), t, g(0), 0));
    ...
    fn budgeted_drain_sends_highest_priority_first() {
        ...
        let t = now();
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 1));
        drop(q.enqueue(vec![0, 2], Some(2), t, g(0), 100));
        drop(q.enqueue(vec![0, 3], Some(3), t, g(0), 50));
    ...
    fn partial_drain_resumes_round_robin_across_calls() {
        ...
        let t = now();
        for gid in 0..3_u64 {
            drop(q.enqueue(vec![0, 1], Some(gid), t, g(gid), 0));
        }
    ...
    fn round_robin_cursor_wraps() {
        ...
        for gid in 0..2_u64 {
            for i in 0..2_u64 {
                drop(q.enqueue(vec![0, 1], Some(gid * 10 + i), t, g(gid), 0));
            }
        }
    ...
    fn byte_budget_evicts_lowest_priority() {
        ...
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 0));
        drop(q.enqueue(vec![0, 2], Some(2), t, g(0), 0));
        drop(q.enqueue(vec![0, 3], Some(3), t, g(0), 0));
        assert_eq!(q.len(), 3);
    ...
    fn byte_budget_evicts_across_groups() {
        ...
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 5));
        drop(q.enqueue(vec![0, 2], Some(2), t, g(1), 1));
    ...
    fn byte_budget_rejects_lower_priority_newcomer() {
        ...
        let t = now();
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 10));
        drop(q.enqueue(vec![0, 2], Some(2), t, g(0), 10));
        drop(q.enqueue(vec![0, 3], Some(3), t, g(0), 10));
        assert_eq!(q.len(), 3);
    ...
    fn byte_budget_admits_equal_priority_newcomer_across_groups() {
        ...
        drop(q.enqueue(vec![0, 1], Some(1), t, g(2), 1));
        drop(q.enqueue(vec![0, 2], Some(2), t, g(1), 1));
        assert_eq!(q.len(), 2);
    ...
    fn byte_budget_evicts_multiple_datagrams_for_one_large_one() {
        ...
        drop(q.enqueue(vec![1], Some(1), t, g(0), 0));
        drop(q.enqueue(vec![2], Some(2), t, g(0), 0));
        drop(q.enqueue(vec![3], Some(3), t, g(0), 0));
        drop(q.enqueue(vec![4], Some(4), t, g(0), 0));
        assert_eq!(q.len(), 4);
    ...
    fn byte_budget_eviction_burst_moves_to_the_next_group_once_one_drains() {
        ...
        drop(q.enqueue(vec![1], Some(1), t, g(0), 0));
        drop(q.enqueue(vec![2], Some(2), t, g(0), 0));
        drop(q.enqueue(vec![3], Some(3), t, g(1), 10));
        drop(q.enqueue(vec![4], Some(4), t, g(1), 10));
        assert_eq!(q.len(), 4);

        // Needs three evictions to fit: both of group 0's order-0 datagrams
        // (the lower-priority bucket, drained as one burst), then one from
        // group 1's order-10 bucket once group 0 no longer exists.
        assert_eq!(
            q.enqueue(vec![9; 67], Some(5), t, g(0), 20),
            DatagramQueueOutcome::Overflowed {
                dropped: vec![1, 2, 3],
                evicted: 3
            }
    ...
    fn byte_budget_stops_evicting_once_what_is_left_outranks_the_newcomer() {
        ...
        drop(q.enqueue(vec![1], Some(1), t, g(0), 0));
        drop(q.enqueue(vec![2], Some(2), t, g(0), 100));
        drop(q.enqueue(vec![3], Some(3), t, g(0), 100));
        drop(q.enqueue(vec![4], Some(4), t, g(0), 100));
    ...
    fn rejection_arms_the_resume_signal() {
        ...
        drop(q.enqueue(vec![1], Some(1), t, g(0), 10));
        assert_eq!(
            q.enqueue(vec![2], Some(2), t, g(0), 0),
            DatagramQueueOutcome::Rejected
        );
    ...
    fn rejection_of_a_large_write_does_not_resume_until_it_would_fit() {
        ...
        drop(q.enqueue(vec![1], Some(1), t, g(0), 100));
        drop(q.enqueue(vec![2], Some(2), t, g(0), 100));
        assert_eq!(
    ...
    fn expiry_alone_releases_a_blocked_queue() {
        ...
        drop(q.set_max_age(Some(Duration::from_millis(10)), t0, NO_DEFAULT));
    ...
    fn max_age_expiry_during_drain() {
        let mut q = DatagramQueue::default();
        let t = now();
        drop(q.set_max_age(Some(Duration::from_millis(50)), t, NO_DEFAULT));

        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 0));
        let t1 = t + Duration::from_millis(80);
        drop(q.enqueue(vec![0, 2], Some(2), t1, g(0), 0));
    ...
    fn max_age_expiry_high_priority_bucket() {
        ...
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 100)); // high priority, will expire
        let t1 = t + Duration::from_millis(80);
        drop(q.enqueue(vec![0, 2], Some(2), t1, g(0), 1)); // low priority, fresh
    ...
    fn round_robin_cursor_advances_under_partial_drain() {
        ...
        let mut q = DatagramQueue::default();
        let t = now();
        drop(q.enqueue(vec![0, 1], Some(100), t, g(0), 0));
        drop(q.enqueue(vec![0, 2], Some(110), t, g(1), 0));
        drop(q.enqueue(vec![0, 3], Some(120), t, g(2), 0));
        drop(q.enqueue(vec![0, 4], Some(101), t, g(0), 0));
        drop(q.enqueue(vec![0, 5], Some(111), t, g(1), 0));
        drop(q.enqueue(vec![0, 6], Some(121), t, g(2), 0));
    ...
    fn peek_next_len_matches_take_next() {
        let mut q = DatagramQueue::default();
        let t = now();
        drop(q.enqueue(vec![0, 1, 2], Some(1), t, g(0), 0));
    ...
    fn peek_and_take_follow_priority_and_round_robin() {
        ...
        let t = now();
        drop(q.enqueue(vec![0, 1], Some(1), t, g(0), 0));
        drop(q.enqueue(vec![0, 2], Some(2), t, g(1), 0));
