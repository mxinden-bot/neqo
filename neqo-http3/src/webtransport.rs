...
pub trait ClientSession {
    ...
    /// Set the outgoing-datagram queue's `outgoingMaxBufferedDatagrams` (an
    /// `unsigned long`, clamped to at least 1 at the `WebIDL` setter per the
    /// WebTransport spec), or clear it back to no limit with `None`. `0` is
    /// deliberately unrepresentable here: a queued datagram is never sent
    /// directly, so a mark of `0` would put the queue permanently above
    /// watermark from the very first enqueue.
    ///
    /// # Errors
    ///
...
impl ClientSession for Http3Client {
    ...
    fn webtransport_set_datagram_max_age(
        ...
        let (conn, handler) = self.connection_and_handler();
        handler.webtransport_set_datagram_max_age(conn, session_id, max_age, now)
    }
...
impl Handler for Http3Connection {
    ...
    fn webtransport_set_datagram_high_water_mark(
        ...
        self.extended_connect_set_datagram_high_water_mark(
            session_id,
            conn,
            high_water_mark.map(NonZeroUsize::get),
        )
    }
    ...
    fn webtransport_set_datagram_max_age(
        ...
        self.extended_connect_set_datagram_max_age(session_id, conn, max_age, now)
    }
}
...
pub(crate) trait ServerHandler {
    ...
    /// `None` means no limit. `outgoingMaxBufferedDatagrams` (an `unsigned
    /// long`) is clamped to at least 1 at the `WebIDL` setter per the
    /// WebTransport spec, so `0` never reaches here from that caller; the
    /// type makes it unrepresentable regardless.
    ///
    /// # Errors
    ///
    /// Returns error if the session ID is invalid or is not a WebTransport session.
...
impl ServerHandler for Http3ServerHandler {
    ...
    fn webtransport_set_datagram_high_water_mark(
        ...
        self.base_handler_mut()
            .webtransport_set_datagram_high_water_mark(conn, session_id, high_water_mark)
    }
    ...
    fn webtransport_set_datagram_max_age(
        ...
        // Shrinking the limit can immediately drop already-queued datagrams,
        // which needs a tick to actually report.
        self.mark_needs_processing();
        self.base_handler_mut()
            .webtransport_set_datagram_max_age(conn, session_id, max_age, now)
    }
}
...
impl ServerSession {
    ...
    pub fn set_datagram_high_water_mark(&self, high_water_mark: Option<NonZeroUsize>) -> Res<()> {
        let session_id = self.stream_handler.stream_id();
        self.stream_handler
            .handler
            .borrow_mut()
    ...
    /// `None` restores the implementation-defined default, which is a
    /// finite RTT-derived bound (see `neqo_transport::default_max_age`),
    /// not "no limit".
    ///
    /// # Errors
    ///
