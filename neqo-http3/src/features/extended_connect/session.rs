...
use neqo_transport::{
    AppError, Connection, DatagramId, DatagramOutcome, DatagramQueueCapacity, DatagramQueueOutcome,
    DatagramTracking, StreamId,
...
impl Session {
    ...
    pub(crate) fn send_datagram<I: Into<DatagramTracking>>(
        ...
        match &outcome {
            ...
            DatagramQueueOutcome::Overflowed { dropped, evicted } => {
                self.report_dropped(*evicted, dropped);
            }
        }
        Ok(outcome)
    }
    ...
    /// still counted here, but need no further per-datagram reporting).
